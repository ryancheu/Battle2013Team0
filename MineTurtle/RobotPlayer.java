...
public class RobotPlayer {
	...
	private static final int NUM_ARMY = 20;
	...
	private static MapLocation[] findWaypoints(RobotController rc, MapLocation start, MapLocation target){
	...
	private static MapLocation findNextWaypoint(RobotController rc, MapLocation[] waypoints){
		int closestWaypoint = -1;
		int closestWaypointDistance = MAX_DIST_SQUARED;
		for(int i=0; i<waypoints.length; i++){
			MapLocation waypoint = waypoints[i];
			int dist = myLocation.distanceSquaredTo(waypoint); 
