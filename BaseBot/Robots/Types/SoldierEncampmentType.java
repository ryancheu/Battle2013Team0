...
public class SoldierEncampmentType {
	...
	private static void findEncampmentStateLogic() throws GameActionException
	{
		...
		//Alright, loop through how many encampments our HQ has barred to make a clean path
		//And store them in spotTaken so that we know NOT to take it.
		//Get number of encampments that we can't use.
		
		int numBannedEncampments= SoldierRobot.mRadio.readChannel(RadioChannels.NUM_BAD_ENCAMPMENTS) ^ FIRST_BYTE_KEY;
		...
		if(numBannedEncampments>0 && numBannedEncampments<10)
		{
			for(int q=0;q<numBannedEncampments;q++)
			{
				int curSpot =SoldierRobot.mRadio.readChannel(RadioChannels.NUM_BAD_ENCAMPMENTS+q+1) ^ FIRST_BYTE_KEY;
				if(curSpot>0)
				{
					spotTaken[curSpot] = true;
				}
			}
		}
