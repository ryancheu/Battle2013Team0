...
public class SoldierProtectLeftEncampmentType {
	...
	private static int protectType = 0;
	...
	private static void armyGotoRallyLogic() throws GameActionException {
		Robot[] enemyRobots = mRC.senseNearbyGameObjects(Robot.class,
				MAX_DIST_SQUARED, SoldierRobot.mEnemy);
		Robot[] alliedRobots = mRC.senseNearbyGameObjects(Robot.class,
		...
		for (Robot arobot : enemyRobots) {
			...
			tempDist = tempRobotInfo.location.distanceSquaredTo(mRC
					.getLocation());
		...
		if (mRC.getEnergon() < SOLDIER_RUN_EVENTUALLY_HEALTH
				...
				&& !indexToLocation(
						SoldierRobot.mRadio
								.readChannel(RadioChannels.MEDBAY_LOCATION))
						.equals(SoldierRobot.HQLoc)) {
			SoldierRobot.switchState(SoldierState.GOTO_MEDBAY);
			return;
		}
		// if we read our position on the BECOME ENCAMPMENT channel, AND we're
		// on an encampment
		else if (SoldierRobot.mRadio
				.readChannel(RadioChannels.BECOME_ENCAMPMENT) == ((mRC
				.getLocation().x + mRC.getLocation().y * mRC.getMapWidth()) | FIRST_BYTE_KEY)
	...
	private static void battleLogic() throws GameActionException {
		Robot[] enemyRobots = mRC.senseNearbyGameObjects(Robot.class,
				MAX_DIST_SQUARED, SoldierRobot.mEnemy);
		Robot[] nearbyEnemyRobots = mRC.senseNearbyGameObjects(Robot.class,
				SOLDIER_JOIN_ATTACK_RAD, SoldierRobot.mEnemy);
		Robot[] alliedRobots = mRC.senseNearbyGameObjects(Robot.class,
				MAX_DIST_SQUARED, SoldierRobot.mTeam);
		...
		for (int i = numEnemies; --i >= 0;) {
			...
			if (tempDist == 3
					&& (mRC.senseEncampmentSquare(tempRobotInfo.location) == false || mRC
							.senseRobotInfo(enemyRobots[i]).type == RobotType.SOLDIER)) {
				if (mRC.senseNearbyGameObjects(Robot.class,
						tempRobotInfo.location, 2, SoldierRobot.mTeam).length == 0) {
		...
		if (closestDist < 3) {
			badLocations = 0;
				...
				&& SoldierRobot.rand.nextFloat() < BREAK_TWO_SQUARES_PROB_NO_NUKE) {
			badLocations = 0;
		} else if (SoldierRobot.enemyNukingFast
				&& SoldierRobot.rand.nextFloat() < BREAK_TWO_SQUARES_PROB_NUKE) {
			badLocations = 0;
		}
		...
		if (hasAllyInFront(closestEnemy)
			...
			if (randomNumber < CHANCE_OF_DEFUSING_NEUTRAL_MINE
					&& (enemyRobots.length < alliedRobots.length / 3)) {
		...
		if (closestDist >= SOLDIER_BATTLE_FORMATION_DIST) {
			...
			MapLocation avg = new MapLocation(
					(enemy.x + mRC.getLocation().x) / 2,
		...
		Direction tempDir;
		if ((tempDir = determineBestBattleDirection(
				getNeighborStats(badLocations), closestEnemy, badLocsTwo)) != null) {
	...
	// Returns least surrounded position or closest position to battle rally, or
	// null if cannot move.85.85
	// badlocsfortwo is for places that are within striking distance of a robot
	// if we move there
	private static Direction determineBestBattleDirection(int[] neighborData,
			MapLocation closestEnemy, int badLocsForTwo)
		...
		float numNearbyEnemies = mRC.senseNearbyGameObjects(Robot.class,
		...
		if (!locallyOutnumbered) {
			for (int i = NUM_DIR; --i >= 0;) {
				...
				if (tempNumEnemies == 0) {
					...
					tempScore = (tempNumEnemies << 1)
							- (1f / distSqrToBattleRally);
				}
				...
				if (neighborData[i] < 100) {
					...
					distSqrToBattleRally = nextToLocations[i]
							.distanceSquaredTo(closestEnemy);
					if (tempNumEnemies == 0) {
						tempScore = NUM_DIR + distSqrToBattleRally;
					} else {
						tempScore = (tempNumEnemies << 1)
								- (1f / distSqrToBattleRally); // multiply by 2
																// to make sure
																// enemy # more
																// important
																// than rally
																// dist
						if (checkForMineBlock(nextToLocations[i],
								DIRECTION_REVERSE[i])) {
							tempScore += 8;
						}
					}
		...
		mRC.setIndicatorString(1, "choose dir:  " + bestDir + "outnubmered: "
				+ locallyOutnumbered + "neigh data " + neighborData[NUM_DIR]
				+ "round" + Clock.getRoundNum());
		// mRC.setIndicatorString(1, "bytecode used for determine: " + (a -
	...
	public static boolean checkForMineBlock(MapLocation mp, Direction fromDir) {
		...
		for (int i = numNonAllyMines; --i >= 0;) {
			tempDiff = (nonAllyMines[i].directionTo(mp).ordinal() - fromDir
					.ordinal());
	...
	// returns the number of enemy/allied robots if a robot were to go in each
	// direction.
	// number of allied is in 10s place, number of enemies is in 1s, a 100 means
	// the direction is blocked
	public static int[] getNeighborStats(int badLocs)
			throws GameActionException {

		enemyThere = new boolean[5][5]; // google tells me this is initialized
										// to false
		// TODO: Make this use a faster arraylist

		Robot[] NearbyRobots = mRC.senseNearbyGameObjects(Robot.class,
				2 * 2 + 2 * 2, ARobot.mEnemy); // 2 in either direction
		...
		for (int i = NUM_DIR; --i >= 0;) {
			...
			if (!isMineDirDanger(tempLoc) && mRC.canMove(tempDir)
					&& ((badLocs >> (i)) & 1) != 1) {

				directionLocs
						.add(new LocationAndIndex(roboLoc.add(tempDir), i));
			} else {
				eachDirectionStats[i] = 100; // This signifies the spot is not
												// movable
			}
		}

		// Go through all the robots and see if they're near any of the squares
		// next to us
		...
		for (int i = nearbyRobotsLength; --i >= 0;) {
			...
			for (j = directionLocsLength; --j >= 0;) {
				if (tempLocation.distanceSquaredTo(directionLocs.get(j).mp) <= 2) { // 2
																					// means
																					// directly
																					// next
																					// to
	...
	private static void gotoMedbayLogic() throws GameActionException {
		...
		if (mRC.getEnergon() < SOLDIER_RETURN_HEALTH) {
			...
			if (!medbay.equals(lastMedbayLoc)) {
				...
				if (SoldierRobot.wayPoints != null
						&& SoldierRobot.wayPoints.size() > 1) {
					medbayWaypoints = convertWaypoints(
							SoldierRobot.wayPoints.toArray(new MapLocation[0]),
			...
			goToLocation(findNextWaypoint(medbayWaypoints, mRC.getLocation()),
					true);
		} else {
			SoldierRobot.switchState(SoldierState.GOTO_RALLY);
		}
	...
	private static void attackHQLogic() throws GameActionException {
		/*
		 * if ( mRC.getEnergon() < SOLDIER_RUN_HEALTH ) {
		 * SoldierRobot.switchState(SoldierState.GOTO_MEDBAY); return; }
		 */
